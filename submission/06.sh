# Calculate the total output value of this transaction in satoshis
raw_tx=""

# Decode the raw transaction and calculate the total output value.
total_output=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq '[.vout[].value] | add')

# Convert the total output value to satoshis (1 BTC = 100,000,000 satoshis).
total_output_satoshis=$(echo "$total_output * 100000000" | bc)

# Output the total output value in satoshis.
echo $total_output_satoshis
