# Calculate the total output value of this transaction in satoshis
raw_tx=""







echo $total_output_value# Output the total output value in satoshistotal_output_value=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq '[.vout[].value] | map(. * 100000000 | floor) | add')# Decode the raw transaction and extract the output values# Decode the raw transaction and extract the output values
total_output_value=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq '[.vout[].value] | map(. * 100000000 | floor) | add')

# Output the total output value in satoshis
echo $total_output_value
